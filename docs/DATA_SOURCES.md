...
> Источник истины по методологии калибровки —
> [`docs/methodology/METHODOLOGY_FINAL.md`](methodology/METHODOLOGY_FINAL.md) §5.
> Четыре группы источников ниже соответствуют Таблице 3 финального документа;
> no-fabrication protocol описан в §12 и продублирован в разделе 6 ниже.
...
| Спектральный радиус $\rho(A)$ | 0.3955 (после rescale off-diag max=0.5) | — |
| Spearman ρ vs A_wiod_v3 | 0.714 | — |
| x_j (Gross Output, DEU+USA, 2014) | energy=246777, water=12949, transport=564730 | `data/calibration/wiod_sector_outputs.json` |
| A* = A·x_j/x_i (Haimes 2005 eq.11) | 3×3, $\rho(A^\star)=0.3955$ (инвариант) | `data/calibration/A_star_iim_canonical.json` |

**Примечание по x_j:** Gross Output (GO/TOT row) читается из второго листа NIOT.
Для RUS сектор water имеет GO=0 (отсутствует в статистике) → агрегирование только по DEU+USA.
Скрипты: `scripts/matrix_calibration/extract_sector_outputs.py`,
`scripts/matrix_calibration/apply_haimes_transformation.py`.
...
## 6. Протокол нехватки данных (no-fabrication protocol)

Соответствует METHODOLOGY_FINAL.md §5.1 и §12 — ограничения методологии.
...
недостающих эмпирических данных. Остановиться и сообщить. Недостающие параметры
помечаются как *к заполнению*, направление сбора данных выносится в перспективы
(METHODOLOGY_FINAL.md §5.1, §12).
