...
def test_interaction_queue_skips_http_errors(monkeypatch: pytest.MonkeyPatch) -> None:
    call_count = {"n": 0}

    async def flaky_apply_step(step, scenario_id, run_id):
        call_count["n"] += 1
        if call_count["n"] == 1:
            raise simulator.HTTPException(status_code=502, detail="unsupported pair")
        return {
            "sector": step.sector,
            "action": step.action,
            "scenario_id": scenario_id,
    ...
    monkeypatch.setattr(simulator, "_apply_step", flaky_apply_step)
